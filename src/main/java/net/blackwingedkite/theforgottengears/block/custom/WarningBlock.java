package net.blackwingedkite.theforgottengears.block.custom;
...
public class WarningBlock extends Block {
    public WarningBlock(Properties properties) {
        super(properties);
    }
    ...
    public InteractionResult use(BlockState state, Level level, BlockPos blockPos, Player player, InteractionHand hand, BlockHitResult blockHitResult) {
        player.sendSystemMessage(Component.literal("危险！"));
    ...
    public void stepOn(Level level, BlockPos blockPos, BlockState blockState, Entity entity) {
        if(entity instanceof LivingEntity livingEntity){
            livingEntity.hurt(new DamageSource("啦啦啦"), 2);
